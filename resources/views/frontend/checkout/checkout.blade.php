                                ...
                                <div class="woocommerce-checkout-review-order">
                                    <table class="table table-bordered">
                                            ...
                                            <tr class="cart-subtotal">
                                                ...
                                                <td><span class="text-danger">&dollar;
                                                    @if (session('coupon_info')->coupon_type == 'Parcentage')
                                                        ...
                                                        <input type="hidden" name="coupon_discount" value="{{ session('coupon_info')->discount }}%" />
                                                    @else
                                                        {{ session('coupon_info')->discount }} <small>({{ session('coupon_info')->coupon_name }})</small>
                                                        <input type="hidden" name="coupon_discount" value="{{ session('coupon_info')->discount }}" />
                                                    @endif
                                        ...
                                        <tr class="delivery">
                                            ...
                                            <td>
                                            <span class="woocommerce-Price-currencySymbol">&dollar; {{ session('shipping_charge') }}</span>
                                                <input type="hidden" name="delivery_change" value="{{ session('shipping_charge') }}" />
                                            </td>
                                        </tr>
                                            ...
                                            <td>
                                                <strong>
                                                    ...
                                                    @if (session('after_discount'))
                                                        ...
                                                        <input type="hidden" name="total_price" value="{{ session('after_discount') + session('shipping_charge') }}" />
                                                        @php
                                                            session(['total' => session('after_discount') + session('shipping_charge')])
                                                        @endphp
                                                    @else
                                                        {{ session('subtotal') + session('shipping_charge') }}
                                                        <input type="hidden" name="total_price" value="{{ session('subtotal') + session('shipping_charge') }}" />
                                                        @php
                                                            session(['total' => session('subtotal') + session('shipping_charge')])
                                                        @endphp
                                                    @endif
                                                </span>
                                            </strong>
                                    ...
                                    <div class="bg-secondary text-white p-3 mb-2 py-3 mt-5">
                                        <div class="form-check mb-2">
                                            ...
                                            <label for="payment_method_cheque" class="form-check-label">Cash On Delivery</label>
                                        </div>
                                        <div class="form-check mb-2">
